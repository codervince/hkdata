    ...
    def parse(self, response):
        ...
        if "Click to open" not in response.body:
            ...
            for row in response.xpath("//table[@class='tableBorderBlue tdAlignC']//table/tr[position()>1]"):
                ...
                l.add_xpath('Jockeyname', './td[7]/a/text()', re=r"[^\d\(\)]+")
                l.add_xpath('Jockeyclaim', './td[7]/a/text()', re=r"\((.+)\)")
                l.add_xpath('Jockeycode', './td[7]/a/@href', re=r"jockeycode=([A-Z]+)")
                l.add_xpath('JockeyWtOver', './td[8]/text()')
                l.add_xpath('Draw', './td[9]/text()')
                l.add_xpath('Trainername', './td[10]/a/text()')
                l.add_xpath('Trainercode', './td[10]/a/@href', re=r"trainercode=([A-Z]+)")
